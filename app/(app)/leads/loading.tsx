      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="h-8 rounded-lg w-28 skel" />
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8].map((row) => (
            <div key={row} className="px-4 py-3 flex items-center gap-4">
              {[16, 12, 14, 12, 12, 10, 8].map((w, col) => (
                <div key={col} className={`h-3.5 rounded ${col === 0 ? 'skel-strong' : 'skel'}`} style={{ width: `${w - ((row * 3 + col * 9) % 4)}%`, animationDelay: `${row * 50 + col * 30}ms` }} />
              ))}
            </div>
          ))}
        </div>
