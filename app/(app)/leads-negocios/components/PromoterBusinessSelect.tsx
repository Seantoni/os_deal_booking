...
import { updateEventLeadPromoterBusiness } from '@/app/actions/event-leads'
import toast from 'react-hot-toast'

interface PromoterBusinessSelectProps {
  eventId: string
  promoterBusinessId: string | null
  promoterBusinessName: string | null
  onSuccess?: () => void
}

const DROPDOWN_HEIGHT = 280 // approximate max height of dropdown

export function PromoterBusinessSelect({
  eventId,
  promoterBusinessId,
  promoterBusinessName,
  onSuccess,
}: PromoterBusinessSelectProps) {
  ...
  useEffect(() => {
    ...
    // Reposition on scroll (capture phase to catch scrolling in any container)
  ...
  const handleSelect = async (business: { id: string; name: string }) => {
    ...
    const result = await updateEventLeadPromoterBusiness(eventId, business.id)
    setUpdating(false)
  ...
  const handleClear = async () => {
    ...
    const result = await updateEventLeadPromoterBusiness(eventId, null)
    setUpdating(false)
  ...
  const displayValue = promoterBusinessName || 'Seleccionar...'
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          businesses.map((business) => {
            const isSelected = promoterBusinessId === business.id
            return (
              <button
                key={business.id}
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={updating}
        className={`w-full text-left px-2 py-1 text-[13px] rounded border transition-colors flex items-center justify-between gap-1 min-w-0 ${
          isOpen 
      ...
      >
        <span className="truncate">
          {updating ? 'Guardando...' : displayValue}
        </span>
        <KeyboardArrowDownIcon
          className={`flex-shrink-0 transition-transform ${isOpen ? 'rotate-180 text-blue-500' : 'text-gray-400'}`}
          style={{ fontSize: 16 }}
        />
      </button>
      {dropdownContent}
