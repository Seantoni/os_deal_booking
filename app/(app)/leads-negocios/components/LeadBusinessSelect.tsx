...
import { updateBankPromoMatch } from '@/app/actions/bank-promos'
import toast from 'react-hot-toast'

export type LeadBusinessSelectEntityType = 'restaurant-lead' | 'bank-promo'

interface LeadBusinessSelectProps {
  entityType: LeadBusinessSelectEntityType
  entityId: string
...
export function LeadBusinessSelect({
  entityType,
  entityId,
  matchedBusinessId,
  matchedBusinessName,
    ...
    async (businessId: string | null) => {
      const result =
        entityType === 'restaurant-lead'
          ? await updateRestaurantLeadMatch(entityId, businessId)
          : await updateBankPromoMatch(entityId, businessId)
      if (result.success) {
        toast.success(businessId ? 'Negocio vinculado' : 'Vínculo eliminado')
        onSuccess?.()
      } else {
      ...
      >
        {matchedBusinessName ? (
          <>
            ...
            {confidencePercent && (
              <span className="text-[10px] text-gray-400 flex-shrink-0">{confidencePercent}%</span>
            )}
          </>
        ) : (
          <span className="truncate">{updating ? 'Guardando...' : 'Vincular...'}</span>
        )}
