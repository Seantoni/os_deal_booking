...
/**
 * Reusable error boundary UI for route-level error.tsx
 */
export default function RouteError({ error, reset, title, context }: RouteErrorProps) {
  useEffect(() => {
    const tag = context || 'route'
    logger.error(`[RouteError:${tag}]`, error.message, error.digest)
  ...
  const pageTitle = title ? `Error en ${title}` : 'Algo salió mal'
