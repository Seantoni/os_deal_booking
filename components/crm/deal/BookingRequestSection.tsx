...
export default function BookingRequestSection({ deal, onViewRequest }: BookingRequestSectionProps) {
  ...
  const endDate = deal.eventDates?.endDate ? new Date(deal.eventDates.endDate) : null
  const todayPanama = parseDateInPanamaTime(getTodayInPanama())

  const startDateKey = startDate ? formatDateForPanama(startDate) : null
  const endDateKey = endDate ? formatDateForPanama(endDate) : null
  const startDatePanama = startDateKey ? parseDateInPanamaTime(startDateKey) : null
  const endDatePanama = endDateKey ? parseDateInPanamaTime(endDateKey) : null

  const daysUntilLaunch = startDatePanama
    ? Math.round((startDatePanama.getTime() - todayPanama.getTime()) / ONE_DAY_MS)
    : null
  const campaignDurationDays = startDatePanama && endDatePanama
    ? Math.round((endDatePanama.getTime() - startDatePanama.getTime()) / ONE_DAY_MS) + 1
    : null
  ...
  const countdownStyles = daysUntilLaunch === 0
    ...
    : daysUntilLaunch !== null && daysUntilLaunch > 0 && daysUntilLaunch <= 7
    ? 'bg-amber-100 text-amber-700 border-amber-200'
    : daysUntilLaunch !== null && daysUntilLaunch > 7
