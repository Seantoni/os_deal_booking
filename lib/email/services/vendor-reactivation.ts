...
export async function sendVendorReactivationEmail(params: SendVendorReactivationEmailParams) {
  ...
  await resend.emails.send({
    ...
    html: renderVendorReactivationEmail({
      ...
      deals: params.eligibleDeals.map((deal) => {
        ...
        return {
          ...
          margin: deal.margin,
          viewUrl: deal.previewUrl || deal.dealUrl || '#',
          replicateUrl: `${baseUrl}/api/vendor-reactivation/replicate?token=${encodeURIComponent(replicateToken)}`,
        }
      }),
    }),
  })
