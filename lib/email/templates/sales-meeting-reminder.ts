 ...
 *
 * Daily reminder for sales users who have not registered meetings today.
 */
...
export function renderSalesMeetingReminderEmail(props: SalesMeetingReminderEmailProps): string {
  ...
  const content = `
    ...
    <p style="margin: 0 0 24px 0; font-size: 16px; line-height: 1.5; color: ${EMAIL_STYLES.colors.text}; text-align: center;">
      Hola <strong>${escapeHtml(userName)}</strong>, al momento no vemos reuniones registradas hoy en tu CRM.
    </p>

    <div style="background-color: #f5f5f7; border-radius: 12px; padding: 24px; margin-bottom: 28px;">
      ${renderKeyValue('Reuniones registradas hoy', String(meetingsCount), true)}
  ...
  return renderEmailLayout({
    ...
    previewText: 'No tienes reuniones registradas hoy en el CRM',
    children: content,
  })
}
