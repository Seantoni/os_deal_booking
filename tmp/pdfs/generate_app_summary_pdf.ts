...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    h1 { margin: 0 0 6px 0; font-size: 17px; line-height: 1.15; color: #0f172a; }
    .subtitle { margin: 0 0 10px 0; color: #475569; font-size: 10px; }
    .section { margin-bottom: 8px; padding: 7px 9px; border: 1px solid #e2e8f0; border-radius: 7px; background: #fff; break-inside: avoid; }
    .section h2 { margin: 0 0 5px 0; font-size: 11px; color: #0f172a; letter-spacing: 0.2px; text-transform: uppercase; }
    p { margin: 0; }
...
<body>
  ...
  <section class="section">
    <h2>What It Is</h2>
    <p>OS Deals Booking is an internal CRM and booking management web app for OfertaSimple built with Next.js App Router, Clerk auth, and Prisma/PostgreSQL. It centralizes sales pipeline, booking requests, deals, calendar events, and operational workflows in one system.</p>
  </section>

  <section class="section">
    <h2>Who It's For</h2>
    <p>Primary persona: OfertaSimple internal operations team, especially Sales/Admin users (plus Editor, ERE, Marketing roles defined in <span class="mono">lib/constants/user-roles.ts</span>).</p>
  </section>
  ...
  <div class="grid">
    ...
    <section class="section">
      ...
      <ul>
        ...
        <li>Set required env vars: <span class="mono">DATABASE_URL</span>, <span class="mono">TOKEN_SECRET_KEY</span>, <span class="mono">OPENAI_API_KEY</span>, <span class="mono">RESEND_API_KEY</span></li>
  ...
  <section class="section">
    ...
    <p class="flow"><strong>UI layer:</strong> Next.js route groups separate authenticated pages (<span class="mono">app/(app)</span>) and public flows (<span class="mono">app/(public)</span>); shared UI in <span class="mono">components/</span>.<br/>
    ...
    <strong>Data flow:</strong> User action -> Next.js page/API -> auth/role checks -> server action/API logic -> Prisma queries/mutations -> side effects (email/AI/external sync) -> UI refresh via cache invalidation/revalidation.</p>
  </section>
...
async function main() {
  ...
  const outPath = resolve('output/pdf/os_deals_booking_app_summary.pdf')
